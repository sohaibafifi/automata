...
namespace Automates
{
    public partial class DfaCreation : Form
    {

        int initial = 0;
        public Dfa UserDfa = new Dfa();
        ...
        private void InitializeComponent()
        {
            ...
            this.Name = "DfaCreation";
            this.Text = "Création de Dfa";
            this.Load += new System.EventHandler(this.Dfa_creation_Load);
        ...
        private void Valide1_Click(object sender, EventArgs e)
        {
            ...
            foreach (object car in Xlist.Items)
                UserDfa.X.Add(car);
            UserDfa.S = (int)Nbetats.Value;
            UserDfa.S0 = initial;
            foreach (object etat in EtatsFList.Items)
                UserDfa.F.Add(ToInt((string)etat));
        ...
        private void supprimer_Click(object sender, EventArgs e)
        {
            if (transitions.SelectedItem != null)
            {
                ...
                UserDfa.RemoveInstruction(trip);
        ...
        private int[] tripletToII(string triplet)
        {
            int[] temp = new int[3];
            temp[0] = -1; temp[1] = -1; temp[2] = -1;
            for (int i = 0; i < UserDfa.S; i++)
                foreach (char car in UserDfa.X)
                {
                    int etatf = UserDfa.getInstruction(i, car);
                    if (String.Equals(triplet, "(S" + i + " , " + car + " ,  S" + etatf + " )"))
                    {
                        temp[0] = i;
                        temp[1] = car;
                        temp[2] = etatf;
        ...
        private void Dfa_creation_Load(object sender, EventArgs e)
        {
